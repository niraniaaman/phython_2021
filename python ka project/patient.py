...
class patient:
    def __init__(self,root):
        ...
        self.bg_home=PhotoImage(file="D:\python ka project\images\home2.png")
        ...
        self.bg_doctoricon=PhotoImage(file="D:\python ka project\images\doctoricon.png")
        ...
        self.bg_patienticon=PhotoImage(file="D:\python ka project\images\patienticon.png")
        ...
        self.bg_appointmenticon=PhotoImage(file="D:\python ka project\images\calendericon.png")
        ...
        self.bg_admiticon=PhotoImage(file="D:\python ka project\images\dmiticon.png")
        ...
        self.bg_exiticon=PhotoImage(file="D:\python ka project\images\exiticon.png")
        ...
        self.patientnametext=StringVar()
        self.patient_name=Label(self.root,text="Patient Name",font=("times new roman",13)).place(x=116,y=118)
        self.patient_name_entry=Entry(frame_patientdetail,font=("times new roman",15),textvariable=self.patientnametext,borderwidth=4,relief=GROOVE)
        ...
        self.dobtext=StringVar()
        self.patient_DOB=Label(self.root,text="D.O.B",font=("times new roman",13)).place(x=125,y=192)
        self.patient_DOB_entry=Entry(frame_patientdetail,font=("times new roman",15),textvariable=self.dobtext,borderwidth=4,relief=GROOVE)
        ...
        self.patient_contact_entry=Entry(frame_patientdetail,font=("times new roman",15),textvariable=self.contacttext,borderwidth=4,relief=GROOVE)
        ...
        self.patient_email_entry=Entry(frame_patientdetail,font=("times new roman",15),textvariable=self.emailtext,borderwidth=4,relief=GROOVE)
        ...
        self.patient_address_entry=Entry(frame_patientdetail,font=("times new roman",15),textvariable=self.addresstext,borderwidth=4,relief=GROOVE)
        ...
        self.symptomtext=StringVar()
        self.symptom=Label(frame_appoint,text="Symptoms",font=("times new roman",13)).place(x=15,y=20)
        self.symptom_entry=Entry(frame_appoint,font=("times new roman",15),textvariable=self.symptomtext,borderwidth=4,relief=GROOVE)
        self.symptom_entry.place(x=130,y=15,width=165,height=90)



        conn=pymysql.connect(host="localhost",user="root",password="Amandeep@14454",database="hospital")
        cur=conn.cursor()
        ...
        self.date=Label(frame_appoint,text="Enter Date (YYYY-MM-DD)",font=("times new roman",13)).place(x=15,y=130)
        self.date_entry=Entry(frame_appoint,font=("times new roman",15),textvariable=self.datetext,borderwidth=4,relief=GROOVE)
        ...
        self.time_entry=Entry(frame_appoint,font=("times new roman",15),textvariable=self.timetext,borderwidth=4,relief=GROOVE)
        ...
        frame_dis=Frame(self.root,borderwidth=1,relief=SOLID) #border
    ...
    def add(self):
        c=self.patient_contact_entry.get().isalpha()
        self.regex = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'
        conn=pymysql.connect(host="localhost",user="root",password="Amandeep@14454",database="hospital")
        cur=conn.cursor()
        if  (self.patient_name_entry.get()=="" or self.clicked.get()=="" or self.patient_DOB_entry.get()=="" or self.patient_contact_entry.get()=="" or self.patient_email_entry.get()=="" or self.patient_address_entry.get()=="" or self.symptom_entry.get()=="" or self.clicked1.get()=="" or self.clicked2.get()=="" or self.date_entry.get()=="" or self.time_entry.get()==""):
            messagebox.showerror("ERROR","All Fields Are Required", parent=self.root)

        elif c==TRUE or len(self.patient_contact_entry.get())!=10:
             messagebox.showerror("ERROR","Invalid Contact No.", parent=self.root)  
             
        else:
            if (re.search(self.regex,self.patient_email_entry.get())==None):
                messagebox.showerror("EMAIL","INVALID EMAIL", parent=self.root)
            else:
    ...
    def search1(self):
        conn=pymysql.connect(host="localhost",user="root",password="Amandeep@14454",database="hospital")
        cur=conn.cursor()
        cur.execute("select ROW_NUMBER() over(order by patient_name) as ID,patient_name,Gender,DOB,Contact,email,Address,Symptoms,Specification,doctor,Apt_date,Apt_time from patient where patient_name=%s",(self.name_entry.get()))
        rows=cur.fetchall()
        if len(rows)!=0 or len(rows)==0:
            self.patient_table.delete(*self.patient_table.get_children())
            for i in rows:
    ...
    def update(self):
        ...
        if  (self.patient_name_entry.get()=="" or self.clicked.get()=="" or self.patient_DOB_entry.get()=="" or self.patient_contact_entry.get()=="" or self.patient_email_entry.get()=="" or self.patient_address_entry.get()=="" or self.symptom_entry.get()=="" or self.clicked1.get()=="" or self.clicked2.get()=="" or self.date_entry.get()=="" or self.time_entry.get()==""):
            messagebox.showerror("ERROR","All Fields Are Required", parent=self.root)
             ...
             messagebox.showerror("ERROR","Contact No. Should be of 10 digit", parent=self.root)  
    ...
    def get_cursor(self,event=" "):
        cursor_rows=self.patient_table.focus()
        content=self.patient_table.item(cursor_rows)
        rows=content["values"]
        self.patientnametext.set(rows[1])
        self.dobtext.set(rows[3])
        self.contacttext.set(rows[4])
        self.emailtext.set(rows[5])
        self.addresstext.set(rows[6])
        self.symptomtext.set(rows[7])
        self.datetext.set(rows[10])
        self.timetext.set(rows[11])
