...
class managedoctor:
    def __init__(self,root):
        ...
        self.name_entry=Entry(frame_adddoctor,font=("times new roman",15),textvariable=self.nametext,borderwidth=4,relief=GROOVE)
        self.name_entry.place(x=132,y=78,width=150)

        
        self.feestext=StringVar()
        self.fee=Label(frame_adddoctor,text="Consultancy Fees",font=("times new roman",13)).place(x=4,y=132)
        self.fee_entry=Entry(frame_adddoctor,font=("times new roman",15),textvariable=self.feestext,borderwidth=4,relief=GROOVE)
        ...
        self.contact_entry=Entry(frame_adddoctor,font=("times new roman",15),textvariable=self.contacttext,borderwidth=4,relief=GROOVE)
        ...
        self.email_entry=Entry(frame_adddoctor,font=("times new roman",15),textvariable=self.emailtext,borderwidth=4,relief=GROOVE)
        ...
        self.address_entry=Entry(frame_adddoctor,font=("times new roman",15),textvariable=self.addresstext,borderwidth=4,relief=GROOVE)
    ...
    def add(self):
        c=self.contact_entry.get().isalpha()
        self.regex = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'
        conn=pymysql.connect(host="localhost",user="root",password="Amandeep@14454",database="hospital")
        cur=conn.cursor()
        if self.name_entry.get()=="" or self.fee_entry.get()=="" or self.contact_entry=="" or self.email_entry=="" or self.address_entry=="":
            messagebox.showerror("ERROR","All Fields Are Required", parent=self.root)
        
        elif c==TRUE or len(self.contact_entry.get())!=10:
            messagebox.showerror("ERROR","Invalid Contact No.", parent=self.root)

        else:
            if (re.search(self.regex,self.email_entry.get())==None):
    ...
    def get_cursor(self,event=" "):
        cursor_rows=self.doctor_table.focus()
        content=self.doctor_table.item(cursor_rows)
        rows=content["values"]
        self.nametext.set(rows[2])
        self.addresstext.set(rows[3])
        self.feestext.set(rows[4])
        self.contacttext.set(rows[5])
        self.emailtext.set(rows[6])
